...
VIDEO_MAX_TOKEN_NUM = int(float(os.environ.get('VIDEO_MAX_TOKEN_NUM', 256)))

FPS = 2.0 
FRAME_FACTOR = 2
FPS_MIN_FRAMES = 4
FPS_MAX_FRAMES = int(float(os.environ.get('FPS_MAX_FRAMES', 180)))
MAX_NUM_WORKERS_FETCH_VIDEO = 8

MODEL_SEQ_LEN = int(float(os.environ.get('MODEL_SEQ_LEN', 12800))) # hyperparameter tested
...
def smart_nframes(
    ...
    if "nframes" in ele:
        ...
        if nframes > total_frames:
            logger.warning(f"smart_nframes: final nframes[{nframes}] > total_frames[{total_frames}] after clamping")
...
def fetch_video(ele: Dict[str, Any], image_patch_size: int = 14, return_video_sample_fps: bool = False,
    ...
    image_factor = image_patch_size * SPATIAL_MERGE_SIZE # e.g., 16*4=64
    VIDEO_FRAME_MIN_PIXELS = VIDEO_MIN_TOKEN_NUM * image_factor * image_factor # 24 * 32 * 32
    VIDEO_FRAME_MAX_PIXELS = VIDEO_MAX_TOKEN_NUM * image_factor * image_factor # 256 * 32 * 32
