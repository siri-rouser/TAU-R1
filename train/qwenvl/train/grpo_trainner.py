...
from train_utils import get_peft_state_non_lora_maybe_zero_3
...
class QwenGRPOTrainer(GRPOTrainer):
    def __init__(self, *args, **kwargs):
        ...
        if getattr(self, "generation_config", None) is not None:
            if getattr(self.generation_config, "temperature", None) is None:
                self.generation_config.temperature = float(getattr(self.args, "temperature", 1.0))
            if getattr(self.generation_config, "top_p", None) is None:
                self.generation_config.top_p = float(getattr(self.args, "top_p", 1.0))

        if not hasattr(self, "max_prompt_length"):
            self.max_prompt_length = getattr(self.args, "max_prompt_length", None)
        if not hasattr(self, "max_completion_length"):
            self.max_completion_length = getattr(self.args, "max_completion_length", None)

    def _is_qwen3_vl_model(self) -> bool:
        config = getattr(self.model, "config", None)
        model_type = str(getattr(config, "model_type", "")).lower()
        architectures = " ".join(getattr(config, "architectures", []) or []).lower()
        model_id = str(getattr(config, "_name_or_path", "")).lower()

        processor_name = type(getattr(self, "processing_class", None)).__name__.lower()
        video_processor_name = type(getattr(getattr(self, "processing_class", None), "video_processor", None)).__name__.lower()

        return (
            "qwen3" in model_type
            or "qwen3" in architectures
    ...
    def _generate_single_turn(self, prompts: list):
        ...
        is_qwen3_vl = self._is_qwen3_vl_model()
        ...
        if images is not None:
            images = [([] if x is None else x) for x in images]
            if all(len(x) == 0 for x in images):
                images = None

        # then only pass if not None
        if images is not None:
            processor_kwargs["images"] = images
            processor_kwargs["do_resize"] = False
    ...
    def _generate_and_score_completions(
        ...
        if "images" in inputs[0]:
            ...
            images = [[ex.get("image")] if ex.get("image") is not None else [] for ex in inputs]
        else:
            images = None

        # Normalize: never allow None entries
        if images is not None:
            images = [([] if x is None else x) for x in images]
            # if every sample has no images, treat as no-images (so we won't pass images kwarg at all)
            if all(len(x) == 0 for x in images):
                images = None
