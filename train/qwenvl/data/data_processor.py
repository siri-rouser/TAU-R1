...
class LazySupervisedDatasetEval(Dataset):
    """Dataset for supervised fine-tuning."""

    def __init__(self, processor, data_args):
        super(LazySupervisedDatasetEval, self).__init__()

        dataset = data_args.eval_dataset_use.split(",")
        dataset_list = data_list(dataset)
        rank0_print(f"Loading datasets: {dataset_list}")
        self.video_max_total_pixels = getattr(
            data_args, "video_max_total_pixels", 1664 * 28 * 28
...
def make_supervised_data_module(processor, data_args) -> Dict:
    ...
    if data_args.eval_dataset_use is not None:
        eval_dataset = LazySupervisedDatasetEval(processor, data_args=data_args)
    else:
        eval_dataset = None
